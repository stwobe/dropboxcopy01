...
# is in fact based upon, note the use of closures and lambda) than a "good" OO
# program.
#
# To run this code get and install OpenGL, GLUT, PyOpenGL
# (see http://www.python.org), and NumPy.
# Installing PyNumeric means having a C compiler that is configured properly,
# or so I found.  For
# Win32 this assumes VC++, I poked through the setup.py for Numeric, and chased
# through disutils code
# and noticed what seemed to be hard coded preferences for VC++ in the case of
# a Win32 OS.  However,
# I am new to Python and know little about disutils, so I may just be not
# using it right.
#
# NumPy is not a hard requirement, as I am led to believe (based on skimming
# PyOpenGL sources) that
# PyOpenGL could run without it. However preformance may be impacted since
# NumPy provides an efficient
...
def InitGL(Width, Height):	  # We call this right after OpenG window is created
...
def ReSizeGLScene(Width, Height):
    ...
    glViewport(0, 0, Width, Height)	  # RCVPT - see note at bottom
    glMatrixMode(GL_PROJECTION)
    glLoadIdentity()
    gluPerspective(45.0, float(Width) / float(Height), 0.1, 100.0)
    glMatrixMode(GL_MODELVIEW)
...
# large values make an unpleasant display with flickering and tearing.
# I found that
...
#  since this is double buffered swap buffers to display what just got drawn.
        glutSwapBuffers()
...
def main():
        ...
        window = glutCreateWindow("Jeff Molofee's GL Code\
         Tutorial ... NeHe '99")

        # Register the drawing function with glut, BUT in Python land,
        # at least using PyOpenGL, we need to
        # set the function pointer and invoke a function to actually register
        # the callback, otherwise it
...
# RCVPT = Reset The Current Viewport And Perspective Transformation